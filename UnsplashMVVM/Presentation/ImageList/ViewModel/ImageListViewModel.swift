...
class ImageListViewModel {
    ...
    func requestPhoto(query: String) {
        UnsplashAPIManager.shared.requestUnsplashPhotoWithAPIRouter(router: APIRouter.get(query: query, page: 1)) { [weak self] value, error in
    ...
    func requestPhotoPagination(query: String, index: Int) {
        ...
        UnsplashAPIManager.shared.requestUnsplashPhotoWithAPIRouter(router: APIRouter.get(query: query, page: currentPage)) { [weak self]  value, error in
